...
public class JComponentWithEvents extends JComponent
		implements MouseListener, MouseMotionListener, KeyListener, ComponentListener, ActionListener, // for timer
...
// Methods to override
	...
	public static final char F1 = KeyEvent.VK_F1, F2 = KeyEvent.VK_F2, F3 = KeyEvent.VK_F3, F4 = KeyEvent.VK_F4,
			F5 = KeyEvent.VK_F5, F6 = KeyEvent.VK_F6, F7 = KeyEvent.VK_F7, F8 = KeyEvent.VK_F8, F9 = KeyEvent.VK_F9,
	...
	private final void runVersionCheckerInstance()
	{
		try
		{
			...
			try
			{
				scanner = new Scanner(new java.net.URL(url).openStream());
			} catch (Exception e)
			{
				if (isApplet)
					return;
	...
	private void showUrlInBrowser(String url)
	{
		...
		else
		{
			Process p = null;
			// windows 1
			if (exec("iexplore.exe", url) != null)
				return;
			// windows 2
			if (exec("\"C:\\Program Files\\Internet Explorer\\IEXPLORE.EXE\"", url) != null)
				return;
			// mac
			if (exec(new String[] { "open", "-a", "Firefox.app", url }) != null)
				return;
			// oh well...
		}
	}
	...
	{
		Dimension newSize = getImageSize(image, scale, radians);
		drawImage(page, image, cx - (int) (newSize.getWidth() / 2), cy - (int) (newSize.getHeight() / 2), scale,
				radians);
	}
	...
	private static java.util.HashMap<String, Image> fileToImageMap = new java.util.HashMap<String, Image>();
	...
	public final void saveBufferedImage(BufferedImage image, String filename)
	{
		...
		if (java.util.Arrays.binarySearch(legalFormats, format) < 0)
			throw new RuntimeException(
					"Illegal format " + format + " not in " + java.util.Arrays.toString(legalFormats));
		try
		{
	...
	private void checkNonLocalVariableValues(HashMap<Field, Object> oldValuesMap)
	{
		...
		for (Field field : this.getClass().getDeclaredFields())
		{
			...
			if ((oldValue != newValue) && ((oldValue == null) || (!oldValue.equals(newValue))))
				setWarningMessage("Warning:  Field changed in paint method!", "Field: " + field.getName(),
						"   Changed from: " + oldValue, "   Changed to: " + newValue,
						"Fields should only be changed in event handlers", "and never in paint methods!");
		}
	}
	...
	private char getKeyEventChar(KeyEvent e)
	{
		...
		if (ignoreModifierKeys && ((key == SHIFT) || (key == CAPS_LOCK) || (key == CONTROL) || (key == ALT)))
			key = (char) 0;
		return key;
	}
